...
class SolrClient {
  constructor(settings) {
    const { onChange, onError } = settings;
    ...
    if (
      this.state.query.pageStrategy === "cursor" &&
      !this.state.query.idField
    ) {
      throw new Error(
        "Pagination strategy 'cursor' requires a unique 'idField' to be passed."
      );
    }
  }
  ...
  setInitialQuery(queryToMerge) {
    ...
    this.state.query.searchFields = this.state.query.searchFields.map(
      sf =>
        searchFieldsToMerge.map(sfm => sfm.field).indexOf(sf.field) > -1
          ? {
              ...sf,
              value: searchFieldsToMerge.find(sfm => sfm.field === sf.field)
                .value
            }
          : sf
    );

    this.state.query.sortFields = this.state.query.sortFields.map(
      sf =>
        sortFieldsToMerge.map(sfm => sfm.field).indexOf(sf.field) > -1
          ? {
              ...sf,
              value: sortFieldsToMerge.find(sfm => sfm.field === sf.field).value
            }
          : sf
    );
  }
  ...
  setSearchFieldValue(field, value) {
    ...
    const newFields = searchFields.map(
      searchField =>
  ...
  setSortFieldValue(field, value) {
    ...
    const newSortFields = sortFields.map(
      sortField =>
